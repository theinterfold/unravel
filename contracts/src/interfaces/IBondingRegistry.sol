...
interface IBondingRegistry {
    ...
    event TicketBalanceUpdated(
        address indexed operator, int256 delta, uint256 newBalance, bytes32 indexed reason
    );
     ...
     */
    event LicenseBondUpdated(address indexed operator, int256 delta, uint256 newBond, bytes32 indexed reason);
     ...
     */
    event SlashedFundsWithdrawn(address indexed to, uint256 ticketAmount, uint256 licenseAmount);
     ...
     */
    function getTicketBalanceAtBlock(address operator, uint256 blockNumber) external view returns (uint256);
     ...
     */
    function pendingExits(address operator) external view returns (uint256 ticket, uint256 license);
     ...
     */
    function previewClaimable(address operator) external view returns (uint256 ticket, uint256 license);
     ...
     */
    function claimExits(uint256 maxTicketAmount, uint256 maxLicenseAmount) external;
     ...
     */
    function slashTicketBalance(address operator, uint256 amount, bytes32 reason) external;
     ...
     * @dev Only callable by authorized slashing manager
     */
    function slashLicenseBond(address operator, uint256 amount, bytes32 reason) external;
     ...
     */
    function distributeRewards(IERC20 rewardToken, address[] calldata operators, uint256[] calldata amounts)
        external;
     ...
     */
    function withdrawSlashedFunds(uint256 ticketAmount, uint256 licenseAmount) external;
}
