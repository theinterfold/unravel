...
interface ICiphernodeRegistry {
    ...
    event CommitteeRequested(
        uint256 indexed e3Id, uint256 seed, uint32[2] threshold, uint256 requestBlock, uint256 committeeDeadline
    );
    ...
    function requestCommittee(uint256 e3Id, uint256 seed, uint32[2] calldata threshold) external returns (bool success);
    ...
    function getCommitteeNodes(uint256 e3Id) external view returns (address[] memory committeeNodes);
    ...
    function setSortitionSubmissionWindow(uint256 _sortitionSubmissionWindow) external;
