...
import {E3, IE3Program, IDecryptionVerifier} from "./IE3.sol";
...
interface IInterfold {
    ...
    /// @dev MUST match the Interfold deployment being targeted. The whole struct is one ABI
    ...
    function publishCiphertextOutput(uint256 e3Id, bytes calldata ciphertextOutput, bytes calldata proof)
        external
    ...
    function setCiphernodeRegistry(ICiphernodeRegistry _ciphernodeRegistry) external;
    ...
    function setDecryptionVerifier(bytes32 encryptionSchemeId, IDecryptionVerifier decryptionVerifier) external;
    ...
    function getDecryptionVerifier(bytes32 encryptionSchemeId) external view returns (IDecryptionVerifier);
    ...
    function checkFailureCondition(uint256 e3Id) external view returns (bool canFail, FailureReason reason);
